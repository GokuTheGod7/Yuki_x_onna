...
BOT_USERNAME = "luna_music_xbot"

@app.on_message(filters.command(["afk", "rb"], prefixes=["/", "!", "b"]))
...
async def love_percentage(_, message: Message):
    if len(message.command) < 3:
        await message.reply_text("Usage: /love <username1> <username2>")
        return

    username1 = message.command[1]
    username2 = message.command[2]

    try:
        user1 = await app.get_users(username1)
        user2 = await app.get_users(username2)
    except Exception:
        await message.reply_text("One or both users not found.")
        return

    if (user1.id == 5860411988 and user2.id == 5645075587) or (user1.id == 5645075587 and user2.id == 5860411988):
        love_percentage = 100
    else:
        love_percentage = random.randint(0, 100)

    user1_link = f"<a href='tg://user?id={user1.id}'>{user1.first_name}</a>"
    user2_link = f"<a href='tg://user?id={user2.id}'>{user2.first_name}</a>"

    message_text = f"{user1_link} ❤️ {user2_link} = {love_percentage}%"
